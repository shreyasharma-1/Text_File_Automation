...
if len(sys.argv) != 4:
    print("Usage: python happiness_calculator.py <likes> <dislikes> <given>")
    sys.exit(1)


likesInput = sys.argv[1]
dislikesInput = sys.argv[2]
givenInput = sys.argv[3]


likes = set(map(int, likesInput.split("-")))
dislikes = set(map(int, dislikesInput.split("-")))
given = list(map(int, givenInput.split("-")))
